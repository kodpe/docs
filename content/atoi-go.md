...
Vous allez apprendre ci-dessous à recoder vous même en **Go** une version simplifiée de la fonction **`Atoi`**.
...
var nb int = (int)(c - 48)
/*
on soustrait 48 au caractere 'c' pour obtenir sa valeur numérique
...
func Atoi(s string) int {
	...
	for i := 0; i < len(s); i++ {
		// ici nb vaut par exemple 123
		
		nb = nb * 10 
		// ici nb vaut 123 * 10 = 1230, 
		
		nb = nb + (int)(s[i] - 48)
		// ici le nouveau zero est remplacé par le prochain chiffre
		// nb vaut par exemple 1230 + 4 = 1234
		
		// on repete la meme chose dans le for pour tous les chiffres
		// en agrandissant nb au fur et à mesure
	}
...
Et voilà bravo votre fonction **`Atoi`** passe maintenant tous les tests :
